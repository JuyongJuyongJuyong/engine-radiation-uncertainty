...
- `getRadiationEstimate()` (`src/index.ts`, step 6, implemented 2026-09-05) is now `async` (`Promise<RadiationOutput>`) — the stub's original synchronous signature was never implementable, since every real source here is a network fetch. This is a shape change per the rule above, though moot today since `engine-system-economics` has no implementation yet to coordinate with.
- **KNOWN GAP, not yet resolved**: `RadiationInput` has no tilt/azimuth field, so `getRadiationEstimate()` can only return `kWh_per_m2_per_year` for a canonical latitude-tilt, equator-facing reference surface (see `index.ts`'s `defaultSurfaceForLatitude()`), not the caller's actual roof. `engine-system-economics` (which does own `RoofMetadata`) must apply its own tilt/azimuth/shading adjustment on top of this number — it must not treat it as already roof-specific. Whoever starts `engine-system-economics` should read `index.ts`'s doc comment on this before wiring the two together, and consider whether `RadiationInput` should grow an optional tilt/azimuth override instead.

## How index.ts bridges monthly-average sources into hourly physics (step 6)
- Neither irradiance source here (NASA POWER, Open-Meteo) publishes real hourly data — only daily/monthly/annual totals — but the Perez transposition chain (steps 1-3) needs hourly resolution. `src/monthlyTransposition.ts` bridges this with the standard Klein & Theilacker (1977) "representative day of the month" method plus the Collares-Pereira & Rabl (1979) daily-to-hourly correlation (both reproduced in Duffie & Beckman, already this repo's core physics reference) — see that module's doc comment for the full method and its disclosed approximation error, which is *not* the same thing as the two irradiance sources' own uncertainty and is folded into `index.ts`'s `SOURCE_RELATIVE_STD_DEV` sanity-checked-not-derived assumption, same category as the rest of this file's disclosed defaults.
- Aerosol correction (step 4) is intentionally not wired into this pipeline yet — see `monthlyTransposition.ts`'s doc comment for why (would need a real hourly/monthly AOD series to be worth doing over this method's own ~1-2% approximation error).
- `tier` is accepted but does not yet change behavior — no Tier 1/2-grade source (PVGIS is CORS-blocked, see `irradianceEnsemble.ts`) is wired in to route to.
